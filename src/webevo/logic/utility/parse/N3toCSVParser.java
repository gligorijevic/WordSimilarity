 ...
 */
package webevo.logic.utility.parse;
...
//                System.out.println(a);
    ...
    private static boolean checkMultiplePredicts(String n3filepath) throws FileNotFoundException, IOException {
        File n3file = new File(n3filepath);
        BufferedReader bufRdr = new BufferedReader(new FileReader(n3file));
        ArrayList<String> predicts = new ArrayList<>();
        String line;
        ...
        while ((line = bufRdr.readLine()) != null) {
            ...
            if (currentSubject == null) {
                currentSubject = elementsInLine[0].trim().substring(1);
            } else if (!currentSubject.equals(elementsInLine[0].trim().substring(1))) {
                return false;
            }
            if (elementsInLine[0].trim().startsWith("<")) {
                String substringedPredict = elementsInLine[1].trim().substring(1);
